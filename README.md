# ArcSight (formerly vision-tools)

A Flask-based web application for industrial computer vision with a modern React frontend. Supports multiple camera types (USB, GenICam, OAK-D, RealSense) with real-time video streaming and configurable vision pipelines (AprilTag detection, colored shape detection, ML object detection).

**New React Frontend:** The web interface has been migrated to React + Vite + TypeScript with optimized bundle sizes for low-power devices like Raspberry Pi. See [REACT_MIGRATION.md](REACT_MIGRATION.md) for details.
...
2. **Run the web server:**
   ...
   **Production mode** (serves pre-built React app):
   ```bash
   ...
   **Development mode** (auto-starts React dev server with HMR):
   ```bash
...
The web interface is built with React + Vite + TypeScript. Frontend code is in the `frontend/` directory.
...
# Option 1: Let Flask auto-start Vite (recommended)
cd ..
export REACT_DEV_MODE=true
python run.py
...
- ✅ Optimized for low-power devices (109KB initial load)
- ✅ shadcn/ui component library
- ✅ Type-safe API client
- ✅ Real-time updates with polling

See [REACT_MIGRATION.md](REACT_MIGRATION.md) for complete frontend documentation.
...
| `REACT_DEV_MODE` | Enable React dev server with HMR | `false` | `true`, `false` |
| `SKIP_VITE_START` | Skip auto-starting Vite (manual start) | `false` | `true`, `false` |
