...
def mock_libs():
    ...
    with patch('app.pipelines.apriltag_pipeline.robotpy_apriltag') as mock_rpa, \
         patch('app.pipelines.apriltag_pipeline.Transform3d', new_callable=MagicMock) as mock_transform3d:
...
def test_process_frame_with_tags(mock_libs, default_config, default_cam_matrix):
    ...
    mock_estimator_instance.estimateOrthogonalIteration.assert_called_once_with(mock_detection, 50)
...
def test_tag_filtering(mock_libs, default_config, default_cam_matrix):
    ...
    mock_detector_instance = MagicMock()
    mock_detector_instance.detect.return_value = [good_tag, bad_hamming_tag, bad_margin_tag]
    mock_rpa.AprilTagDetector.return_value = mock_detector_instance
    
    mock_estimator_instance = MagicMock()
    mock_estimator_instance.estimateOrthogonalIteration.return_value = create_mock_pose_estimate()
    mock_rpa.AprilTagPoseEstimator.return_value = mock_estimator_instance
...
def test_grayscale_conversion(mock_cvt_color, mock_libs, default_config, default_cam_matrix):
