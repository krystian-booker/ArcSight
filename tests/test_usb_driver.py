...
def test_connect_invalid_identifier(mock_camera_data):
    ...
    # Act & Assert
    with pytest.raises(ConnectionError, match="Invalid identifier for USB camera: 'not-a-number'"):
        driver.connect()
...
def test_list_devices(mock_video_capture):
    ...
    assert devices[0] == {'identifier': '0', 'name': 'USB Camera 0', 'camera_type': 'USB'}
    assert devices[1] == {'identifier': '2', 'name': 'USB Camera 2', 'camera_type': 'USB'}
