...
MOCK_GENICAM_DEVICES = [{'identifier': 'SERIAL123', 'name': 'Test GenICam (SERIAL123)', 'camera_type': 'GenICam'}]
MOCK_OAKD_DEVICES = [{'identifier': 'MXID456', 'name': 'OAK-D MXID456', 'camera_type': 'OAK-D'}]


# --- Tests for discover_cameras ---

@patch('app.camera_discovery.OAKDDriver.list_devices', return_value=MOCK_OAKD_DEVICES)
@patch('app.camera_discovery.GenICamDriver.list_devices', return_value=MOCK_GENICAM_DEVICES)
...
@patch('app.camera_discovery.USBDriver.list_devices', side_effect=RuntimeError("Driver failed"))
