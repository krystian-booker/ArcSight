...
def test_connect_success(mock_dai, oakd_driver):
    ...
    mock_device.getOutputQueue.assert_called_once_with(name="rgb", maxSize=4, blocking=False)
...
def test_connect_device_raises_exception(mock_dai, oakd_driver):
    ...
    # Act & Assert
    with pytest.raises(ConnectionError, match="Failed to connect to OAK-D camera 12345_mxid: Device not found"):
        oakd_driver.connect()
...
def test_list_devices_success(mock_dai):
    ...
    mock_dai.Device.getAllAvailableDevices.return_value = [mock_dev_info1, mock_dev_info2]
    ...
    assert devices[0] == {'identifier': 'mxid_1', 'name': 'OAK-D mxid_1', 'camera_type': 'OAK-D'}
    assert devices[1] == {'identifier': 'mxid_2', 'name': 'OAK-D mxid_2', 'camera_type': 'OAK-D'}

@patch('app.drivers.oakd_driver.dai')
def test_list_devices_raises_exception(mock_dai):
    """Test list_devices when the underlying library call fails."""
    mock_dai.Device.getAllAvailableDevices.side_effect = RuntimeError("Failed to query devices")
    devices = OAKDDriver.list_devices()
    assert devices == []
