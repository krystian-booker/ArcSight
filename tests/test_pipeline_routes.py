...
def test_update_pipeline_config_valid(client):
    ...
    camera = Camera(
        name="Test Camera", identifier="test_valid_0", camera_type="USB", orientation=0
    )
    ...
    # Valid configuration
    valid_config = {"family": "tag36h11", "tag_size_m": 0.165, "threads": 4}
...
def test_update_pipeline_config_type_mismatch(client):
    ...
    camera = Camera(
        name="Test Camera",
        identifier="test_type_mismatch_0",
        camera_type="USB",
        orientation=0,
    )
    db.session.add(camera)
    ...
    invalid_config = {"threads": "not_a_number"}
...
def test_add_pipeline_uses_default_config(client):
    ...
    camera = Camera(
        name="Test Camera",
        identifier="test_default_config_0",
        camera_type="USB",
        orientation=0,
    )
    db.session.add(camera)
    ...
    response = client.post(
        ...
        data=json.dumps({"name": "New Pipeline", "pipeline_type": "AprilTag"}),
        content_type="application/json",
    )
...
def test_update_ml_pipeline_config_invalid_filename(client):
    ...
    camera = Camera(
        name="Test Camera",
        identifier="test_ml_invalid_filename_0",
        camera_type="USB",
        orientation=0,
    )
    db.session.add(camera)
    ...
    malicious_config = {"model_filename": "../../../etc/passwd"}
