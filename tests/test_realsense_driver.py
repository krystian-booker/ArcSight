...
def test_initialization_with_defaults(app):
    ...
    # Use a regular object instead of MagicMock to properly test getattr with defaults
    class MinimalCamera:
        identifier = "test123"
        # Don't define other attributes so getattr returns the defaults

    driver = RealSenseDriver(MinimalCamera())
...
def test_disconnect_when_connected(realsense_driver):
    ...
    realsense_driver.pipeline_started = True  # Mark pipeline as started so stop() gets called
...
def test_get_frame_success_without_depth(mock_np, mock_rs, mock_camera_data):
    ...
    # Assert
    # When depth is disabled, get_frame returns a single numpy array, not a tuple
