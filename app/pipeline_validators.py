...
APRILTAG_SCHEMA = {
    ...
    "properties": {
        "family": {
            "type": "string",
            "enum": ["tag16h5", "tag25h9", "tag36h11", "tagCircle21h7", "tagCircle49h12",
                     "tagCustom48h12", "tagStandard41h12", "tagStandard52h13",
                     "16h5", "25h9", "36h11", "Circle21h7", "Circle49h12",
...
def validate_array_constraints(value: list, constraints: Dict, path: str, schema: Dict) -> None:
...
def validate_value(value: Any, property_schema: Dict, path: str, parent_schema: Dict) -> None:
...
def validate_pipeline_config(pipeline_type: str, config: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    """
    ...
    if not isinstance(config, dict):
        return False, f"Config must be an object/dictionary, got {type(config).__name__}"
    ...
    try:
        ...
        for key, value in config.items():
            if key in schema["properties"]:
                ...
                return False, f"Unknown property '{key}' not allowed for {pipeline_type} pipeline"
