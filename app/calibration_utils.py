...
def generate_charuco_board_pdf(buffer, p, page_size=A4):
    ...
    try:
        ...
        img_size_px = (p['squares_x'] * pixels_per_square, p['squares_y'] * pixels_per_square)
        
        # Generate the board image with a small margin
        board_img = board.generateImage(img_size_px, marginSize=int(pixels_per_square * 0.1), borderBits=1)
        ...
        c.drawImage(img_reader, x_offset_mm * mm, y_offset_mm * mm, width=board_width_mm * mm, height=board_height_mm * mm, preserveAspectRatio=True)
        c.showPage()
        c.save()
